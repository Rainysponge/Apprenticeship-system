...
def update_teacher_comment(request):
    comment_form = Teacher_CommentForm(request.POST)
    if comment_form.is_valid():
        ...
        # comment.content_object = Homework.objects.get(pk=object_id)
        comment.text = comment_form.cleaned_data['text']
        comment.content_object = Teacher.objects.get(pk=comment_form.cleaned_data['teacher_id'])
        comment.save()
